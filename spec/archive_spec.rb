  ...
  describe '.initialize' do
    ...
    context 'local_path omitted' do
      it 'should use Dir.pwd' do
        expect(LoadFile::Archive.new('spec/fixtures/test.zip').local_path).to eq(Dir.pwd)
      end
    end
    ...
    context 'local_path is given' do
      it 'should have local_path set' do
        location = '/tmp'
        expect(LoadFile::Archive.new('spec/fixtures/test.zip',location).local_path).to eq(location)
      end
    end
  end
    ...
    %w(gzip zip).each do |type|
      context "unpacking #{type} archive" do
        it 'should return LoadFile::Status::Success' do
          Dir.mktmpdir do |dir|
            result = LoadFile::Archive.new("spec/fixtures/test.#{type}", dir).unpack
